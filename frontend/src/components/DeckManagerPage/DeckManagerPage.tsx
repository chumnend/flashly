...
const DeckManagerPage = () => {
    ...
    const [cardForm, setCardForm] = useState<CardForm>({ frontText: '', backText: '', difficulty: 'easy' });
    ...
    const openEditCard = (card: Card) => {
        setCardForm({ frontText: card.frontText, backText: card.backText, difficulty: card.difficulty });
        setCardFormError(null);
    ...
    const handleSaveCard = async () => {
        ...
        if (cardModalMode === 'add') {
            ...
            const result = await Flashly.updateCard(deckId, editingCardId, token, cardForm);
            if ('error' in result) {
                setCardFormError(result.error);
            } else {
    ...
    if (deckLoading) {
        return <div className="dm__loading"><div className="dm__spinner" /></div>;
    }

    if (deckError || !deck) {
        return (
            <div className="dm__loading">
                <p className="dm__error-text">⚠ {deckError ?? 'Deck not found'}</p>
                <Link to="/decks" className="dm__back-link">← Back to decks</Link>
            </div>
        );
    }
    ...
    return (
        <div className="dm">
            ...
            <div className="dm__topbar">
                <Link to="/decks" className="dm__back-link">← My Decks</Link>
                <div className="dm__topbar-actions">
                    <Link to={`/decks/${deckId}`} className="dm__preview-link">Preview →</Link>
                    <button className="dm__danger-btn" onClick={() => setConfirmDelete(true)}>
                        Delete deck
                    </button>
                </div>
            </div>
            ...
            <div className="dm__body">
                ...
                <section className="dm__section">
                    <div className="dm__section-head">
                        ...
                        {!deckEditing && (
                            <button className="dm__ghost-btn" onClick={() => setDeckEditing(true)}>Edit</button>
                        )}
                    </div>
                    ...
                    {deckEditing ? (
                        <div className="dm__form">
                            ...
                            <div className="dm__field">
                                ...
                                <div className="dm__toggle">
                                    ...
                                    <button
                                        ...
                                        onClick={() => setDeckForm(f => ({ ...f, publishStatus: s }))}
                                    >
                            ...
                            {deckSaveError && <p className="dm__form-error">{deckSaveError}</p>}
                            <div className="dm__form-actions">
                                <button className="dm__ghost-btn" onClick={() => setDeckEditing(false)}>Cancel</button>
                                <button className="dm__primary-btn" onClick={handleSaveDeck} disabled={deckSaving}>
                        ...
                        <div className="dm__deck-display">
                            ...
                            {deck.description && <p className="dm__deck-desc">{deck.description}</p>}
                            <span className={`dm__badge dm__badge--${deck.publishStatus}`}>
                            {deck.publishStatus}
                            </span>
                        </div>
                    )}
                ...
                <section className="dm__section">
                    ...
                    <h2 className="dm__section-title">
                        Cards
                        <span className="dm__count-pill">{cards.length}</span>
                    </h2>
                    <button className="dm__primary-btn" onClick={openAddCard}>+ Add card</button>
                    </div>

                    {cardsLoading && (
                    <div className="dm__cards-loading"><div className="dm__spinner" /></div>
                    )}
                    ...
                    <div className="dm__empty-cards">
                        ...
                        <p className="dm__empty-sub">Hit "Add card" to build your deck.</p>
                    </div>
                    )}
                    ...
                    <ul className="dm__card-list">
                        ...
                        >
                            ...
                            <div className="dm__card-side">
                                <span className="dm__side-label">Front</span>
                                <p className="dm__side-text">{card.frontText}</p>
                            </div>
                            <div className="dm__card-divider" />
                            <div className="dm__card-side">
                                <span className="dm__side-label">Back</span>
                                <p className="dm__side-text">{card.backText}</p>
                            </div>
                            </div>
                            <div className="dm__card-footer">
                            <span className={`dm__diff dm__diff--${card.difficulty}`}>
                                {card.difficulty}
                            </span>
                            <div className="dm__card-btns">
                                <button className="dm__card-btn" onClick={() => openEditCard(card)}>
                                Edit
                                </button>
                                <button
            ...
            {cardModal && (
                <div className="dm__overlay" onClick={closeCardModal}>
                    <div className="dm__modal" onClick={e => e.stopPropagation()}>
                        <div className="dm__modal-header">
                            <h2 className="dm__modal-title">
                                {cardModalMode === 'add' ? 'Add card' : 'Edit card'}
                            </h2>
                            <button className="dm__modal-close" onClick={closeCardModal}>✕</button>
                        </div>
                        ...
                        <div className="dm__modal-body">
                            <div className="dm__field">
                                ...
                                <textarea
                                    ...
                                    onChange={e => setCardForm(f => ({ ...f, frontText: e.target.value }))}
                                    rows={3}
                                    autoFocus
                                />
                            </div>
                            <div className="dm__field">
                                ...
                                <textarea
                                    ...
                                    onChange={e => setCardForm(f => ({ ...f, backText: e.target.value }))}
                                    rows={3}
                                />
                            </div>
                            <div className="dm__field">
                                ...
                                <div className="dm__toggle">
                                    ...
                                    <button
                                        ...
                                        onClick={() => setCardForm(f => ({ ...f, difficulty: d }))}
                                    >
                            ...
                            {cardFormError && <p className="dm__form-error">{cardFormError}</p>}
                        </div>

                        <div className="dm__modal-footer">
                            <button className="dm__ghost-btn" onClick={closeCardModal}>Cancel</button>
                            <button className="dm__primary-btn" onClick={handleSaveCard} disabled={cardSaving}>
                            {cardSaving ? 'Saving…' : cardModalMode === 'add' ? 'Add card' : 'Save changes'}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {/*  Delete deck confirm  */}
            {confirmDelete && (
                <div className="dm__overlay" onClick={() => setConfirmDelete(false)}>
                    <div className="dm__modal dm__modal--sm" onClick={e => e.stopPropagation()}>
                        <div className="dm__modal-header">
                            <h2 className="dm__modal-title">Delete deck?</h2>
                            <button className="dm__modal-close" onClick={() => setConfirmDelete(false)}>✕</button>
                        </div>
                        <div className="dm__modal-body">
                            <p className="dm__modal-warn">
                                <strong>{deck.name}</strong> and all its cards will be permanently deleted. This cannot be undone.
                            </p>
                        </div>
                        <div className="dm__modal-footer">
                            <button className="dm__ghost-btn" onClick={() => setConfirmDelete(false)}>Cancel</button>
                            <button className="dm__danger-btn" onClick={handleDeleteDeck} disabled={deleting}>
