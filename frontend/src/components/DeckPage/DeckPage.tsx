...
const DeckPage = () => {
    ...
    if (loading) {
        return <div className="dp__loading"><div className="dp__spinner" /></div>;
    }
    ...
    if (error || !deck) {
        return (
            <div className="dp__loading">
                ...
                <Link to="/explore" className="dp__text-link">← Back to explore</Link>
            </div>
        );
    }

    const currentCard = shuffled[currentIndex];
    const progress = shuffled.length > 0 ? ((currentIndex + 1) / shuffled.length) * 100 : 0;
        ...
        return (
            <div className="dp dp--study">
                <div className="dp__study-header">
                    <button className="dp__text-link" onClick={() => setStudyState('idle')}>
                        ← Exit
                    </button>
                    ...
                    <div className="dp__diff-badge dp__diff-badge--sm">
                        <span className={`dp__diff dp__diff--${currentCard.difficulty}`}>
                ...
                <div className="dp__progress-track">
                    <div className="dp__progress-bar" style={{ width: `${progress}%` }} />
                </div>

                {/* Flip card */}
                <div className="dp__card-scene" onClick={() => setFlipped(f => !f)}>
                    <div className={`dp__card-3d ${flipped ? 'dp__card-3d--flipped' : ''}`}>
                        <div className="dp__card-face dp__card-face--front">
                            <span className="dp__face-label">Front</span>
                            <p className="dp__card-text">{currentCard.frontText}</p>
                        ...
                        <div className="dp__card-face dp__card-face--back">
                            ...
                            <p className="dp__card-text">{currentCard.backText}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="dp__study-nav">
                    ...
                    <button className="dp__nav-btn dp__nav-btn--next" onClick={handleNext}>
                        {currentIndex + 1 === shuffled.length ? 'Finish' : 'Next →'}
                    </button>
                </div>
            </div>
        );
        ...
        return (
            <div className="dp dp--complete">
                <div className="dp__complete-card">
                    ...
                    <p className="dp__complete-sub">You reviewed all {shuffled.length} cards.</p>
                    <div className="dp__complete-actions">
                        <button className="dp__primary-btn" onClick={startStudy}>Study again</button>
                        <button className="dp__ghost-btn" onClick={() => setStudyState('idle')}>
                            Back to deck
                        </button>
                    </div>
                </div>
    ...
    return (
        <div className="dp">
            ...
            <div className="dp__header">
                <Link to="/explore" className="dp__text-link">← Explore</Link>
                {isOwner && (
                    <Link to={`/decks/${deckId}/manage`} className="dp__manage-link">
                        Manage deck →
                    </Link>
                )}
            </div>
            ...
            <div className="dp__body">
                ...
                <div className="dp__hero">
                    <div className="dp__hero-meta">
                        <span className={`dp__badge dp__badge--${deck.publishStatus}`}>
                            {deck.publishStatus}
                        </span>
                        {deck.categories && deck.categories.length > 0 && (
                            <div className="dp__tags">
                                {deck.categories.map(c => (
                                    <span key={c.id} className="dp__tag">{c.name}</span>
                                ))}
                            </div>
                        )}
                    </div>
                    <h1 className="dp__title">{deck.name}</h1>
                    {deck.description && <p className="dp__desc">{deck.description}</p>}

                    <div className="dp__stats">
                        <div className="dp__stat">
                            <span className="dp__stat-value">{cards.length}</span>
                            <span className="dp__stat-label">cards</span>
                        </div>
                        {deck.rating > 0 && (
                            <div className="dp__stat">
                                <span className="dp__stat-value">{deck.rating.toFixed(1)}</span>
                                ...
                                >
                                    <div className="dp__list-sides">
                                        <div className="dp__list-side">
                                            <span className="dp__side-label">Front</span>
                                            <p className="dp__side-text">{card.frontText}</p>
                                        </div>
                                        <div className="dp__list-divider" />
                                        <div className="dp__list-side">
                                            <span className="dp__side-label">Back</span>
                                            <p className="dp__side-text">{card.backText}</p>
                                        </div>
                                    </div>
                                    <span className={`dp__diff dp__diff--${card.difficulty}`}>
                                        {card.difficulty}
                                    </span>
                                </li>
                            ))}
                ...
                {cards.length === 0 && (
                    <div className="dp__empty">
                        <p className="dp__empty-title">No cards in this deck yet.</p>
                    </div>
                )}
            </div>
        </div>
